...
void ksetupInterrupts(){
    ...
    kbuildInterruptGate(&stepDescriptor, 0x08, (uint32_t)&_handlerClock, 0x00);
    kmemcpy((uint8_t*)&stepDescriptor, (uint8_t*)(IDT_BASE + 32 * sizeof(interruptGateDescriptor)), sizeof(interruptGateDescriptor));

    kbuildInterruptGate(&stepDescriptor, 0x08, (uint32_t)&_handlerKeyboard, 0x00);
    kmemcpy((uint8_t*)&stepDescriptor, (uint8_t*)(IDT_BASE + 33 * sizeof(interruptGateDescriptor)), sizeof(interruptGateDescriptor));

    kbuildInterruptGate(&stepDescriptor, 0x08, (uint32_t)&_handlerSystemCall, 0x00);
    kmemcpy((uint8_t*)&stepDescriptor, (uint8_t*)(IDT_BASE + 0x80 * sizeof(interruptGateDescriptor)), sizeof(interruptGateDescriptor));
...
void kbuildInterruptGate(interruptGateDescriptor* OUT_gate, uint16_t segSelector, uint32_t offset, uint8_t dpl){
    OUT_gate->offsetLow = offset & 0x0000FFFF;
    OUT_gate->offsetHigh = (offset & 0xFFFF0000) >> 16;
    OUT_gate->segSelector = segSelector;
    OUT_gate->options = 0x00000000;
    OUT_gate->options |= 1 << 15;               // P flag = segment present
    OUT_gate->options |= (dpl & 0b11) << 13;    // DPL
    OUT_gate->options |= 0b111 << 9;            // Type and D flag = 1 size = 32 bits
}
